    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl px-6 pt-32 pb-20 text-center">
        <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-surface/60 px-4 py-1.5 text-xs font-mono text-muted backdrop-blur-sm">
          ...
          <EditableText siteKey="hero_badge" fallback="AI Generation & Automation Lab" />
        </div>

        <h1 className="mx-auto max-w-4xl text-4xl font-bold leading-tight tracking-tight sm:text-5xl md:text-6xl lg:text-7xl">
          <EditableText
            siteKey="hero_title_line1"
            fallback="スマホ・Mac・低スペックPCから"
            className="inline-block text-gradient"
          />
          <br />
          <EditableText
            siteKey="hero_title_line2"
            fallback="ブラウザで手軽に動く"
            className="inline-block text-foreground"
          />
          <br />
          <EditableText
            siteKey="hero_title_line3"
            fallback="商用AI画像・動画生成スタジオ"
            className="inline-block text-gradient"
          />
        </h1>
        ...
        <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-neon-pink/30 bg-neon-pink/10 px-4 py-1.5 text-xs font-mono font-medium text-neon-pink">
          <Gift size={14} />
          <EditableText
            siteKey="hero_signup_banner"
            fallback="新規アカウント登録で即時10クレジット無料進呈（クレカ登録不要）"
          />
        </div>

        <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <EditableLink
            siteKey="hero_cta_primary_href"
            fallback="#studio"
        ...
        <div className="mt-10">
          <EditableMedia
            ...
            kind="image"
            alt="Hero visual"
            className="mx-auto max-h-64 w-full max-w-2xl overflow-hidden rounded-2xl"
          />
        </div>
