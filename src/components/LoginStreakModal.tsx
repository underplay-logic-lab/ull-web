...
export type LoginStreakData = {
  bonus: number;
  streak: number;
  // Position (1-7) within the current 7-day free-tier cycle. null for paid
  // tiers, which get a flat daily amount instead of a cyclical one.
...
export function LoginStreakModal({ data, onClose }: LoginStreakModalProps) {
  ...
  const isFree = data.tier === "free" && data.dayInCycle != null;
    ...
    >
      ...
      >
        ...
        {isFree ? (
          ...
          <div className="mt-6 rounded-2xl border-gradient bg-surface/60 p-6 text-center">
            <p className="font-mono text-xs uppercase tracking-widest text-neon-violet">
              {TIER_LABEL[data.tier] ?? data.tier} MEMBER
            </p>
            <p className="mt-2 text-3xl font-bold text-gradient">+{data.bonus} Credits</p>
            <p className="mt-2 text-sm text-foreground/80">{data.streak}日連続ログイン中！</p>
          </div>
        )}
