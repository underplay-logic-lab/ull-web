...
export const KNOB_META: Record<KnobKey, KnobMeta> = {
  ...
  angle_priority_parallel_rate: {
    ...
    // 2026-09-23 ホスト決定: **固定でよい（率は 0）**。一度「2 倍」案も出たが、
    // Multi-Angle を並列で使う人は少ない見込みで、固定 340C（混雑料金）に落ち着いた。
    // 比例にしたくなったらここ（または admin）を 1.0 等に上げるだけ。
    value: 0,
  ...
  angle_priority_parallel_surcharge: {
    ...
    // 同日、形を「固定 + 通常料金 × 率」に変更したが、最終的に**固定 340C のみ**で
    // 確定（率 0）。DB の既存行も 340。
    value: 340,
