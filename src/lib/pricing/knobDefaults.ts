...
export const KNOB_META: Record<KnobKey, KnobMeta> = {
  ...
  angle_priority_parallel_rate: {
    // 並列実行の上乗せ = ceil(通常料金 × これ) + 固定分。
    // 2026-09-23 ホスト最終決定: **率 1.0（合計 2 倍）+ 固定 50C**。並列が奪うのは
    // GPU 時間ではなく「枠（10 台）の同時占有」なので、占有時間の代理である通常料金に
    // 比例させる。固定 50C はコールドスタート実費（約 13C）の markup 分。固定だけだと
    // 大きいジョブほど相対的に安くなり、枠を長く占有する人ほど並列を選びやすくなる。
    value: 1.0,
  ...
  angle_priority_parallel_surcharge: {
    ...
    // 同日、形を「固定 + 通常料金 × 率」に変更し、最終的に **率 1.0 + 固定 50C** で確定。
    // DB の行も 50 に更新済み（2026-09-23）。
    value: 50,
  ...
  lora_prep_load_s: {
    ...
    // 合計 550秒 の方が実測として確か。
    // 2026-09-20（夜）さらに更新。GUI 既定条件（実効バッチ1 / compile **有効**）
    // の本番ランでログに出た prep 内訳（docs §14.15）:
    //   model load（逆量子化含む） 621.5s
    //   first-step JIT/compile     476.9s   ← compile 有効だと §14.13 の 99.1s から激増
    //   latent キャッシュ          174.4s（131枚 = 1.33 s/枚）
    //   合計 1,272.8s
    // §14.13 は compile **無効**のランだったため first-step が 99.1s で済んで
  ...
  lora_prep_per_image_s: {
    ...
    //
    // 同日さらに GUI 既定条件のラン（docs §14.15）で 174.4秒 / 131枚
    // = **1.33 秒/枚**。§14.13 と同じ131枚で 0.92 -> 1.33 に増えており、
    // latent キャッシュ自体にもランごとのばらつきがある。過小側は原価割れに
    // 直結するので、2点のうち大きい方を採る。
    value: 1.33,
  ...
  lora_batch_marginal_ratio: {
    // 1ステップの所要秒のうち「1 step で処理する枚数（batch_size ×
    // gradient_accumulation）に比例する分」の割合。
    // s/it = 基準値 × 解像度係数 × ((1-これ) + これ × 枚数)。
    // 1.0 で正比例、0 でバッチを上げても所要秒が増えない。
    //
    // 2026-09-21 実測で **ほぼ正比例**と確定（docs/gpu-benchmarks.md §14.8.1、
    // minimax_h3 / B300 / 1024px / rank64 / gc無効 / block_compile / 実写131枚）:
    //   batch1 → 1.78 s/it（1.78秒/枚）
    //   batch2 → 3.45 s/it（1.725秒/枚）
    //   batch4 → 6.52 s/it（1.63秒/枚）
    // バッチ4倍で1画像あたりの改善は9%。バッチ1の時点で既に計算律速のため、
    // 「GPU が遊んでいるからまとめても増えない」という以前の前提は誤りだった
    // （その根拠にしていたベンチ値も s/it 計測バグで無効）。
    // 1.0 のままにする＝実測比で最大9%の過大見積もり＝安全側。
    value: 1.0,
  ...
  lora_spi_baseline_default: {
    ...
    //
    // 2026-09-20（夜・3回目）0.65 → 2.0。GUI 既定条件の実測（docs §14.15）で
    // minimax_h3 が 1.80 になり、また意図が逆転していた（0.65 < 1.80）。
    // 最重量 1.80 をやや上回る値として 2.0 を置く。
    value: 2.0,
  ...
  usd_jpy_rate: {
    // 2026-09-20: 150 → 160。本番DBの行は 2026-09-18 に 160 へ更新されていたが、
