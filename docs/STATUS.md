...
**残り**
00000000. **（手作業の診断が固まってから）LoRA データセットの「おまかせで整える」ボタン（2026-09-25 ホスト案）。**
   診断欄の一番上に置き、①同じ構図の重複を 2 枚残して除外（2 人写りも、2 人とも多いとき）②足りない顔アップ・上半身を
   目安まで切り出す（2 人写りは消す前に切り出し元に）③切り出しの品質で自動除外（顔欠け・2 人以上）④要約と除外画像の一覧・
   **元に戻す**。すぐには消さず「除外」として脇に置く。学習回数はキャプション後に「構図の偏りを均す」を自動でかける。
   個別の操作（減らす・切り出す）は今のまま残す。**先に手作業の流れを詰める（ホスト判断）。**
