...
000000000. **LoRA 既定値の検証（2026-09-26 起票・最優先）。** GUI 既定（prodigy・1 step 1 枚・枚数から自動の step）で焼いた
   ...
   250 ごと保存、YAML はホストの Downloads `hitozuma_kocho_minimax_v3_adamw.yaml`）。1750/2000 を v1 と比べて最適化手法の差を切り分け。 **v1（prodigy）の途中版の結果（ホスト確認 2026-09-26）**: 750 実写人物（未学習）／1000 kocho（取り違え）／**1250 hitozuma の特徴しっかり**／1500・1750 アニメの女性程度／1906 別人。= prodigy は定数の大きい歩幅で 1250 付近の狭いピークを通り過ぎて崩れる（「足りない」ではなく「行き過ぎ」）。既定は安定性優先（ホスト「ピンポイントより安定して似る方が安心」）。
