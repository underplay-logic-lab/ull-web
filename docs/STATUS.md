...
- **Multi-Angle の並列実行 追加料金は「混雑料金」として高めにする**（2026-09-23 ホスト方針）。B200 の実コールド原価
  ...
  原価ベースで下げる提案はしない。**最終決定（2026-09-23 夜）: 固定 340C のみ**（一度「2 倍」案が出て
  「通常料金 × 率 + 固定」の形にしたが、率は 0 で確定。並列で使う人は少ない見込み）。knob `angle_priority_parallel_rate`
  は 0、`angle_priority_parallel_surcharge` は 340。モーダル表記は「通常料金に +340C、合計 △C」。
  マイグレーション `supabase/migrations/20260882000000_angle_parallel_rate_knob.sql` は率の行（0）を足すだけで、
  未適用でも同じ挙動。**1 構図単価は 12 → 14C にホストが admin で変更済み（同夜）。**
