...
000000. **ローンチ前にやる運用まわり（2026-09-24 ホスト指摘）**:
   ...
   **→ 実装（2026-09-25、コミット済み・未 push）**: ホスト判断で「学習回数はキャプションの後」に変更（被写体別の比率は
   ...
   他ワーカーには次のデプロイで入る。キュレーションは必須化（チェックボックス廃止）・推奨枚数は「1 人 15 枚以上」に・.txt だけの後入れに対応。**次: push（Vercel 反映）。**
