...
**残り**
0. **超解像バッチの上限撤廃と LoRA→超解像の受け渡し — 実装済み・未コミット・未デプロイ（2026-09-24）。**
   ...
   まとめて処理に並べ、短辺 768 に届く最小倍率を初期値にする。**反映順: modal deploy（seedvr2）→ Vercel → admin で
   knob を 10800 へ（DB は 1800 のまま）。** その後 100 枚程度で実測して上限を確定する（要承認）。
