...
0. **超解像バッチの上限撤廃と LoRA→超解像の受け渡し — 本番反映済み（2026-09-24、`02c2108`・seedvr2 デプロイ・knob 10800 済み）。**
   ...
   **LoRA 素材の WebP 変換を R2 経路では廃止（2026-09-24）。** 根拠だった Modal 送信の律速が R2 では無い。localStorage
   `ull_lora_upload_webp=1` で変換を強制できるので、同じ PNG データセットで `[lora-upload]` 行（送信秒・変換秒）を比べて確定する。**→ 実測（2026-09-24）: PNG 直 145 枚 / 729.6MB を 17.3 秒（352.8 Mbps・R2・145 リクエスト並列）。WebP は変換だけで 259 枚 1〜2 分＝145 枚で 30〜70 秒の見込みで、送信がゼロでも PNG 直に負けるため比較せず PNG 直で確定。**
