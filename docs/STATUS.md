...
**【完了 2026-09-23】LoCon は既定から外した**（ホスト判断「v6 で十分」、同条件比較済み）。`SDXL_CONV_DIM`
既定 0、`LORA_SPI_BASELINE.sdxl` 1.25 → 0.80（v6 実測 0.74）。kocho が女性っぽくなる原因は推論側の
ネガティブ「醜い」だった（gpu-benchmarks §14.25）。SDXL の固定既定は cosine / tag dropout 0.1 / min_snr 5 の 3 つ。
