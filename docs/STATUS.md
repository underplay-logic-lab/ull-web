...
| 学習時間 | 約44分 |
| 全体 | 約50分（コールドスタート3分＋latentキャッシュ3分＋学習44分） |
| GPU 実費 | 約 ¥260（L40S $1.95/h × 0.83h × 160円） |
| 課金 | 551C = ¥915 相当 → 粗利率 約71% |

**`LORA_SPI_BASELINE.sdxl = 0.642` の校正は不要**。実測 0.74 とのズレは15%で、
想定の粗利率76%に対し実績71%。以前スモークで出た 1.197 のほうが実態と乖離して
