...
最終更新: 2026-09-24（深夜・R2 計画 1〜9 完了、寄付、次の一手を棚卸し）

**再開の合言葉: 「ULL再開」**（ホストがこれだけ打ったら、Claude は質問せずに下の手順で動く）
1. この節の直下「## 次の一手」を読む（古い節を読んで誤報告した前科あり。**「次の一手」以外の節は経緯**）。
2. 「残り」の上から 3 つを 1 行ずつ提示し、どれをやるか聞く（ホストが番号や名前を指定していればそのまま着手）。
3. 着手前に該当領域の docs / メモリを grep して現物を確認してから手を動かす。
...
- LoRA Studio で選択できるモデルは **全部実測済み**: ai-toolkit 6 arch（minimax_h3 §14.15、wan22_14b / ltx2 / krea2 /
  flux2_klein_4b / zimage / anima §14.16〜14.22）＋ SDXL 2 本（juggernaut_xl / illustrious_xl / WAI、§14.23〜14.25・14.28）。
  実行 tier も確定（§14.26〜14.28: wan22/minimax = B300、ltx2/krea2 = H200、klein/zimage/anima/SDXL = RTX PRO 6000）。
- Multi-Angle の単価は **14C**（DB knob `angle_pro_per_angle`、2026-09-23 ホストが admin で変更。コード既定も 14 に揃えた 2026-09-24）。
  サブ参照係数 0.7・並列上乗せ「×1.0 + 50C」も確定。
...
0000000000000000. **課金まわり（2026-09-26）**: プラン変更・同じプランの買い直し＝今の契約を即時 revoke → 新規チェックアウト（即日付与・
   ...
   **ホストの実機テスト済み（2026-09-26）**: Entry→Studio（Developer Test）で Entry が自動終了し Studio に。¥500 チャージ（Studio 50%）で
   実決済: `tax_amount 0`（Polar は今は日本の消費税を取っていない）・手数料 $0.71（Starter 5%+50¢+日本カード 1.5% と一致）・+300C。
   購入後の戻り先は `#studio`＋トースト。**要確認**: ポータルでの解約が期間末でなく即時終了になった（¥0 契約だからか。実契約の
   最初の解約で `ends_at` が期間末か見る）。ポータルは英語のみ（Polar の仕様、チェックアウトは日本語）。
   minimax v8（yukipas v5 の設定・kch3 21 枚）も**プロンプトを無視**（室内で喋る映像ばかり）。v7 と同じ＝設定ではなく素材が原因の
   ~~見込み: 背景が原因~~ → ホスト指摘で撤回: hitozuma も全部白背景で、2 人一緒（88 枚）なら kocho も雪の中に出た（ただし hitozuma も
   「笑って走る」までは従わない）。差は 1 枚あたりの周回数（kch3 単独 3000/21≒143 周、2 人一緒 3000/88≒34 周）と、静止画だけでは
   動作（走る・笑う）が弱い点と見る。背景の警告は根拠が無いので作らない。
   **→ 既定値を確定（2026-09-26）**: `lora-minimax-defaults`（prodigy＋cosine・1000＋24×枚数）を main へ（`c80d520`）、LoRA worker デプロイ済み。
   検証はここで打ち切り（ホスト方針: ローンチ優先）。未確認: LoRA 強度 0.6〜0.8 でプロンプトに従うか（ホスト側で見られれば FAQ に足す）。
...
0000000000000. **2026-09-26 の LoRA Studio 画面まわり（全部 push 済み）**: 生 YAML は「学習設定と LoRA 名だけ差し替える」位置付けに
   ...
   おまかせの後は結果欄に [元に戻す] と行き先名のボタン（キャプションを作る／学習設定へ進む、足りない構図だけ残れば
   マルチアングル、減らす系が残れば手作業で修正）、おまかせは赤の指摘があるときだけ、注意だけなら「このまま進む」、
   判定中表示を目立たせ・未判定が残る間は診断欄へ送らない、学習回数の欄ごと光らせる（学習設定へ進むは除く）。
   R2 転送中の「checkpoint not found」修正（worker デプロイ済み）。**未確認**: おまかせの実機（顔欠け除外含む）、超解像の中止。
   残の小課題: ~~除外した画像（excludedImages）が再読み込みで消える~~ 不具合ではない（LoRA の画像そのものが再読み込みで消える仕様・2026-09-26 確認）／生 YAML の batch>1 の見積もり（minimax は 1 枚あたり約 3 倍遅い、admin しか使わないので後回し＝ホスト判断）。
00000000000. **→ 実装（2026-09-26、push 済み・実機未確認）**: 学習中でも確認画面まで進め、「学習を開始」で追加料金の確認（knob `lora_priority_parallel_rate` 1.0 / `_surcharge` 50C、他タブと同じ式、ホスト判断「2 倍出しても急ぐ人向けの選択肢」）。API は実行中の学習があると `priority` 無しを 409 で弾き、上乗せは請求だけ（損切りは通常料金から）。追っていない学習は画面上部「並列で出した学習」（`LoraBackgroundJobs.tsx`、localStorage）から切り替え。以下は元の記述: **LoRA の同一ユーザー並列実行（2026-09-26 起票）。** CLAUDE.md §6-7 では
   scaledown_window=2 系（LoRA）は追加料金なしで並列を提供する標準だが、`LoraStudioTab.tsx` は進行中ジョブを 1 本しか追えず
   （`inFlightJob` があると送信を止める・二重送信防止を兼ねる、`ACTIVE_JOB_STORAGE_KEY` も 1 件）、v3 実行中に v4 を投げられなかった。
   対応は複数ジョブの追跡（一覧・各ジョブの進捗/完了パネル・リロード復元を配列に）＋二重送信防止は送信中ロックだけに。
000000000. **LoRA 既定値の検証（2026-09-26 起票・最優先）。** GUI 既定（prodigy・1 step 1 枚・枚数から自動の step）で焼いた
   ...
   250 ごと保存、YAML はホストの Downloads `hitozuma_kocho_minimax_v3_adamw.yaml`）。1750/2000 を v1 と比べて最適化手法の差を切り分け。 **v1（prodigy）の途中版の結果（ホスト確認 2026-09-26）**: 500 実写人物（未学習）／750 kocho（取り違え）／**1000 hitozuma の特徴しっかり**／1250・1500 アニメの女性程度／1906 別人。= prodigy は定数の大きい歩幅で 1000 付近の狭いピークを通り過ぎて崩れる（「足りない」ではなく「行き過ぎ」）。既定は安定性優先（ホスト「ピンポイントより安定して似る方が安心」）。 **v3（adamw 1e-4・batch1・rank32・3000 step、`5aadfc1f`、s/it 0.31）**: 〜2000 実写人物（未学習）／**2250 出現**／2500 靴だけリアルな半リアル／**2750・3000 出現**、3000 でも崩れず伸びしろあり（ホスト「3000 以上もありかも」）。= 遅いが安定。**v4（prodigy＋cosine・1500 step）**: 全部アニメ調だが似方はいまいち、1500 が一番まし（学習不足＝cosine で後半の歩幅が落ち、v1 のピーク到達前に止まった）。補足（ホスト）: v1 の 1250 は素材に無い着物でも本人＝2 人の分離が良い、v3 の 2250〜3000 は hitozuma に kocho のスーツ・革靴が滲む。**v5（prodigy＋cosine・3000 step）** へ。 **v5 の結果**: 1500 スーツ／1750・2000 革靴パンツ（kocho の服が滲む）／**2250・2500 ロングスカート・2750 ミニスカートの hitozuma（滲みなし）**／3000 kocho → **seed・名前を変えて確認（ホスト）: 〜2250・2750 はどちらの名前でも hitozuma（kocho 未学習）、2500 は kocho で 2 人、3000（最終）で hitozuma→hitozuma・kocho→kocho と分離**。枚数の少ない kocho（31 枚）は最後に分かれる。cosine は最終版が当たりになりやすい。→ 式を 1000＋24×枚数（88 枚 → 3,112）に修正（ブランチ `9bc0bb0`）。v5 の最終 3000 で kocho 指定 4 回中 2 回 kocho（分離は半分）。次: **v6 = kocho だけ 21 枚**（ホスト「やるなら出にくい kocho。hitozuma は Illustrious でも簡単に出る」、`dataset_88img (1).zip` から kocho 単独 21 枚を抜いた `kocho_solo_21img.zip`、prodigy＋cosine・新しい式で 1,504 step、YAML `kocho_solo_minimax_v6_prodigy_cosine.yaml`）。その後、名前を造語にした 2 人の実験で「名前の意味」か「男性の偏り」かを切り分け。 **2026-09-26 17:07 に v7 投入（ジョブ `kch3_solo_minimax_v7_prodigy_cosine3000`、21 枚・3000 step、17:40 頃完了見込み）→ ホストが途中版と最終版で kch3 の出現を確認中。** 結果が良ければローカルブランチ `lora-minimax-defaults`（`45d65bc`＋`9bc0bb0`: minimax だけ prodigy＋cosine・1000＋24×枚数 step）を main へ入れて LoRA worker をデプロイ（他 6 モデルの確認は 2026-09-26 の判断で見送り、上の 00000000000000）。 **v6 の結果**: 最終（1504）だけ kocho が出現、途中版は出ない＝ぎりぎり届いた（v5 の kocho 50% と合わせ、今の式は kocho には足りない）。次: **v7**（ホスト指摘「刻む必要は無い。3000〜4000 で 1 回、名前も造語にすれば 1 回で済む」で 1 本にまとめた）= 名前を kch3 に置換した `kch3_solo_21img.zip`・prodigy＋cosine・3000 step（`kch3_solo_minimax_v7_prodigy_cosine3000.yaml`）。**既定の変更はローカルブランチ `lora-minimax-defaults`（`45d65bc`、未 push・未デプロイ）に用意済み**: worker の GUI 経路で prodigy のとき minimax_h3 だけ cosine（`PRODIGY_COSINE_ARCHES`）、自動 step は 1000＋20×枚数（`loraCredits.ts`）。seed 違いの確認で安定していれば main へ入れて worker デプロイ。トリガーワード欄に「意味の無い造語が無難」の案内を追加済み（`3e2ce52`）。 **v1（prodigy）の途中版の結果（ホスト確認 2026-09-26）**: 750 実写人物（未学習）／1000 kocho（取り違え）／**1250 hitozuma の特徴しっかり**／1500・1750 アニメの女性程度／1906 別人。= prodigy は定数の大きい歩幅で 1250 付近の狭いピークを通り過ぎて崩れる（「足りない」ではなく「行き過ぎ」）。既定は安定性優先（ホスト「ピンポイントより安定して似る方が安心」）。
...
00000000. **LoRA データセットの「おまかせで整える」ボタン（2026-09-25 ホスト案 → 同日実装、実機未確認）。**
   ...
   手持ちから切り出す（除外候補も切り出し元に使ってから除外）。B. 切り出しの構図判定を待って、2 人以上写っている
   切り出しを除外し、赤の「多すぎる構図」を減らす（量は切り出し後の診断で決める）。C. キャプション完了で「均す」を自動
   （自分で用意する場合は B の直後）。除外は消さずに `excludedImages` へ退避（`AutoTidyPanel` で 1 枚ずつ／全部戻す）。
   減らす候補の選び方は手作業と共通化（`pickDupFirst` / `trimPoolFor` / `duoPoolFor` / `sameCompositionRest`、
   減らす案は `buildTrimPlan` / `buildDuoPlan`）。**顔欠けの自動除外も実装（2026-09-26）**: `runSmartCrop` の出力に `headCut`（顔アップ＝顔の枠が元画像の端から 15% 超はみ出す、上半身＝画像に収めた枠より推定頭頂部が上）。おまかせと手作業の切り出しの両方で不採用にし、内訳に「顔・頭が枠から欠けるので除外 N」。実画像での確認はまだ。
   **次: ホストが hitozuma+kocho で実機確認**（除外枚数・切り出し枚数・元に戻すが正しく動くか）。
0000000. **実稼働ログの原価・売上ずれの修正（2026-09-25、push・適用済み `20260890000000`）。**
   始まっていないジョブは実行時間 0・gpu_tier 'none'（原価 0）、返金済みは credits 0（job_id 列を足し、後からの返金も反映）。
...
000000. **ローンチ前にやる運用まわり（2026-09-24 ホスト指摘）**:
   ...
   ③ **キャプション解析を自前 VLM（Qwen3.8-27B・transformers・B300・64 枚まとめ）へ切り替え済み（2026-09-24、vLLM は冷えた起動が 3〜4 分で撤回）**（docs/gpu-benchmarks.md §17）。
   ...
   **有料化（2026-09-25 ホスト判断）**: 「LoRA に最適化したキャプションを作成（N 枚・XC）」＝ knob `lora_caption_base` 50C
   ...
   特徴を変えた後の作り直しは既存の警告に料金を明記（注意喚起で OK、ホスト判断）。
   **次（方針確定 2026-09-25）: 順番の改修** — ドロップ → 特徴確定 → 診断（WD タガー・CPU・無料。構図タグ full body /
   upper body / close-up 等で判定）→ クロップ → 学習回数 → **キュレーション直前に LoRA 最適化キャプションを 1 回だけ（有料）**
   → キュレーション → 学習開始。最初に「キャプション: AI に作らせる（有料）／自分で書く（無料・全部埋まるまで学習不可）」を選ぶ。
   支払いはキャプションと学習で別々。**残: 特徴抽出・翻訳など他の Gemini 用途は未移行／JoyCaption は Volume に残置。**
   **→ 実装（2026-09-25、push 済み）**: ホスト判断で「学習回数はキャプションの後」に変更（被写体別の比率は
   キャプションで決まるため）。確定した順番: ドロップ → 「取り込み完了（無料）」＝特徴抽出＋構図判定 → 構図診断
   （データセット全体。被写体別はキャプション後）→ クロップ → キャプション（AI・有料 1 回／自分で書く）→ 被写体別診断・
   学習回数 → キュレーション → 学習。構図判定は `modal_wd_tagger.py`（app `ull-wd-tagger`・CPU 8 コア・
   wd-eva02-large ONNX を image に焼き込み・12 枚ずつ並列、**デプロイ済み**）＋ route `/api/studio/lora/wd-tags`
   ＋ `tagDatasetComposition`（`loraCaption.ts`）。通しテスト: 640px JPEG 24 枚で依頼→完了 43s（起動 5〜7s・1 枚 1.4s）。
   診断は構図＝WD タグ・被写体＝キャプション（`DiagnosticInput.tags`／`WHOLE_DATASET_SUBJECT`）。「自分で書く」は
   キュレーション必須・空キャプションが残ると確定不可（AI 側も同じ条件にした）。
   ⚠️ `ull_r2.client()` をロックで保護（冷えたコンテナで 8 スレッドが同時に boto3 client を作り 1 本が 600s 固まった）。
   他ワーカーには次のデプロイで入る。キュレーションは必須化（チェックボックス廃止）・推奨枚数は「1 人 15 枚以上」に・.txt だけの後入れに対応。
   **複数人物を全モデルへ（2026-09-25、ホスト判断「SDXL だけに分ける必要は無い」）**: 人物欄（性別/人数・説明・特徴・
   ...
   短縮・デプロイ済み。**→ 済み（2026-09-24、docs/gpu-benchmarks.md §18）: RTX PRO 6000 へ変更（1 枚 11.5 → 3.6 秒、原価 約 1.6 倍でも誤差）。**
...
0000. **Multi-Angle の「余分な GPU コンテナ」調査 — 結論（2026-09-24）**: 普段の運用では発生しない。09-23 21:32 にジョブ実行中に
   ...
   ジョブ完了で `[angle] container GPU: … / host RAM peak=…GB` を出すようにした（デプロイ済み）。**次の Multi-Angle 1 回で
   実測 → `memory=` を指定する。** → **実測 76.4GB（B200・3 構図、読み込み時がピーク、ジョブ中は増えず）→ `memory=96GiB` を一度指定したが、確保分にメモリ料金（$0.008/GiB/h）が掛かり GPU 時給の約 +12% になるため**外した**（2026-09-24、ホスト判断）。exit 137 は実行中デプロイの先回り起動だけで起きており、デプロイ規律で対処済み。他ワーカーの `memory=` 指定も同じ理由で不要（実測ログは残す）。** 運用ルール「GPU ワーカーはジョブが動いていない時にデプロイ」を CLAUDE.md に追加。
   **全 GPU ワーカーに展開（2026-09-24、デプロイ済み）**: 本番の GPU 関数はどれも `memory=` 未指定（直近 7 日の exit 137 は
   Multi-Angle の 4 件のみ）。LoRA（ai-toolkit / SDXL）・Director・超解像にも `_host_ram_peak_gb()` を入れ、完了時の
   `metadata.host_ram_peak_gb`（DB）とログに出す（LoRA は stage 1 完了時、超解像は ComfyUI 起動時も）。
   ~~各ワーカーで実測が揃ったら `memory=` を指定する~~ **不要（2026-09-24 ホスト判断、上の Multi-Angle の結論と同じ: メモリ料金で GPU 時給 +12%、exit 137 はデプロイ規律で対処済み）。**
   並行化（保存を裏へ）は、1 構図 35〜50 秒のほとんどが推論なので効果 1 割未満の見込み → 後回し。
**GPU は生成に集中させ、保存・転送は裏で並行する（2026-09-24 ホスト方針、全ワーカーへ展開予定）。**
   ...
   残りは 1 枚 1 秒弱で、ほぼ GPU の計算だけ。**次: 同じ形を Multi-Angle・Director・LoRA へ展開する**（どれも「生成 → 保存 → 完了」が直列）。 → **LoRA: 実測で準備段階に約 78s の GPU 待ち（ステージング 58s・zip 18s）→ 並行化してデプロイ済み（2026-09-24、docs §14.30）。klein で確認済み: ステージング 58s→0.2s・stage 1 59s→3s・zip 18s→0.8s。Director: 効果 1〜2% で見送り。Multi-Angle: 1 割未満で見送り。** 2026-09-25 に LoRA（minimax・1906 step）のログで再確認: 途中保存は 1 回 約 3 秒×7 回（学習 12 分 44 秒の 3%）、commit は学習と並行。残る GPU 待ちは学習後の commit＋永続化 約 45 秒（B300 で 1 回 約 $0.09）だけ。案 B（GPU から R2 へ直接）で消せるが効果が小さいので保留。
   併せて案 B（数 MB の結果は Volume を通さず GPU から R2 へ直接）も検討。
**AI（Gemini）原価の計測 — 実装済み・マイグレーション適用済み（2026-09-25 に `ai_usage_logs` への記録を確認）。** Gemini は無料枠ではなく**有料枠**で動いている
   ...
   Vercel ログにも `[gemini-usage]` 行を出す（`20260889000000_create_ai_usage_logs.sql` は適用済み）。次: 145 枚 1 データセット分の合計トークン × Google 単価で 1 回あたりの
...
0. **超解像バッチの上限撤廃と LoRA→超解像の受け渡し — 本番反映済み（2026-09-24、`02c2108`・seedvr2 デプロイ・knob 10800 済み）。**
   ...
   45 分→4h。route は寸法を先頭 256KB の Range 読み・16 並列に、署名 URL の TTL を推定秒 +1h に（後ろの画像の期限切れ防止）、
   アップロード 8 並列・ポーリングを `.in()` 1 クエリに。LoRA の「✨ この N 枚を超解像で拡大する」は該当画像を
   まとめて処理に並べ、短辺 768 に届く最小倍率を初期値にする。~~残: 100 枚程度で実測~~ **閉じた（2026-09-26）**: 100 枚は §18 で実測済み（RTX PRO 6000 で約 4.5 分）、以後の売上÷原価は admin「Pricing」の見積もりと実績の表で見る。
   **追加（2026-09-24）: 超解像の結果を LoRA へ自動で差し戻す。** 完了後に「拡大した N 枚を LoRA Studio に戻して
   差し替える」→ 元画質 PNG（無ければ配信用）を取得し、LoRA 側の同じ id の画像を置き換える（学習回数・キャプションは保持、
   寸法判定は prepareDatasetImage）。対応表はページ内だけ（再読み込みで LoRA の画像も消えるため）。**LoRA → 超解像 → LoRA の往復はホストが実機で正常動作を確認（2026-09-24）。**
   **LoRA 素材の WebP 変換を R2 経路では廃止（2026-09-24）。** 根拠だった Modal 送信の律速が R2 では無い。localStorage
   `ull_lora_upload_webp=1` で変換を強制できるので、同じ PNG データセットで `[lora-upload]` 行（送信秒・変換秒）を比べて確定する。**→ 実測（2026-09-24）: PNG 直 145 枚 / 729.6MB を 17.3 秒（352.8 Mbps・R2・145 リクエスト並列）。WebP は変換だけで 259 枚 1〜2 分＝145 枚で 30〜70 秒の見込みで、送信がゼロでも PNG 直に負けるため比較せず PNG 直で確定。** **サーバー側 Smart Ingest も WebP q95 → PNG（劣化なし）へ変更・デプロイ済み（2026-09-24、縮小は維持、docs §15）。次の学習で prep 時間を旧と比べる。**
1. **Polar API の 2026-10 移行 — 着手不能（Polar 側待ち）。** 2026-04 固定は済み。Polar がまだ 2026-10 版のチェンジログを
   公開していない（2026-09-14 調査、メモリ `polar-api-version-migration`）。**月 1 回、公開されたか見るだけ**。期限は 2027-01。
...
5. **GPU tier の選択制 — 実装済み（2026-09-24）。** LoRA Studio に「標準／高速」の 2 択（高速 tier がある arch だけ表示）。
...
10. **保留（需要が見えたら）: 外部モデルの URL 取り込み**（2026-09-24 ホスト判断）。Civitai の SDXL 派生などを貼るとサーバーが直接取り込んで
   ...
   ①ユーザー LoRA を生成側で使える経路がまだ minimax だけ ②「GPU・モデル名を隠す」運用と外部モデル持ち込みは相性が悪い。
   **自作 LoRA を使う運用と合わせ、メンバー制の専用サービスにするかどうかと一緒に検討する。**
...
**残り（この順で）**
0. **Multi-Angle サブ参照の実測 完了（2026-09-23 夜）**: サブ 0/1/2/3 枚 = 22.5 / 35.2 / 51.7 / 62.7 s/構図
   （×1 / 1.56 / 2.30 / 2.79、ほぼ線形 +50〜70%/枚）、VRAM 56.5 → 58.8GB。**課金係数 0.7 は 4〜11% 安全側で据え置き。**
   詳細は gpu-benchmarks「サブ参照枚数 × 生成時間」。単価 12C（3 構図ジョブで粗利 59%、定常 66%）の見直しは
   ホスト判断待ち（案: 14C で定常 71%）。
   - 同夜の UI 修正（`a5054a2` 以降）: ①順番待ち予約を **全 4 タブ**（Multi-Angle / 超解像 画像・動画 / Director）で
     先入れ先出しのリストに（1 件だと後の予約が前を黙って上書きし、ホストがサブ 1/2/3 枚を予約したら最後だけ走った）。
     ②Multi-Angle に **「今回の生成」一覧**を追加。予約・並列で続けて出したジョブだけを localStorage の並び
     （`multi-angle-session-jobs`）で持ち、`GET /api/studio/angle/jobs` の結果をその id で絞って出す。**改めて生成する
     ときは「これまでの生成結果は消去されます。必要なものは先に DL」と確認して一覧を空にする**（ホスト方針: サーバー側の
     保持は安全弁で、ユーザーには「都度 DL しなければ消える」感覚でいてもらう。**保持期間は UI に書かない**。
     プライバシーポリシーに「一時ストレージ（最長14日間）…破棄」の記述あり）。順番待ちの ZIP 自動 DL は廃止
     （ホストの DL フォルダに無かった。手動の「一括ZIPダウンロード」は使えたので CORS ではなく、当該ジョブが完了の 26 秒後に
     手動で開始されていて予約の自動発火を通らなかった可能性が高い。いずれにせよ廃止）。
     **消去のタイミング（ホスト指示）**: 確認モーダルは出すが、その場では消さない。改めて生成したジョブが**完了した時点**で
     前の並びを消し、そのジョブ 1 件から始める（`freshJobIdRef`）。
     **超解像 画像/動画・Director も同方式に統一**（共有部品 `StudioSessionList.tsx`: 作成時のメタ情報を localStorage に
     並べ、ジョブ本体は各タブの poll で読み直す。自動 DL は 3 タブとも廃止）。
...
### LoRA の「結果がいまいちな時」ヒント（2026-09-23 ホスト発案 → 2026-09-26 ②を実装）

**②は実装済み**: LoRA 完了画面（進行状況パネルの下）に折りたたみ「うまく出ないときは」。先頭に「プロンプトにトリガーワードを必ず入れる（自動書き直しツールで消えていないか）」を追加（ホストが 2026-09-25 に実際に踏んだ: QwenVL の書き直しで名前が消え、強度 3 でも別人）、以下 2・3・5・4 の順。①の FAQ ページはローンチ時。
...
- **Multi-Angle の並列実行 追加料金は「混雑料金」として高めにする**（2026-09-23 ホスト方針）。B200 の実コールド原価
  （約 13C）より大幅に高いのは意図的（「急ぎたいなら金を払え」。安いと皆が並列を使い GPU 枠が枯渇する）。
  原価ベースで下げる提案はしない。**最終決定（2026-09-23 夜）: 通常料金 × 1.0（合計 2 倍）+ 固定 50C**。
