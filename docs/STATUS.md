...
**残り**
1. **Polar API の 2026-10 移行（期限付き）。** 現在 2026-04 固定。2027 年 1 月のローテーションで削除される。メモリ `polar-api-version-migration`。
2. **LoRA「結果がいまいちな時」ヒント＋FAQ ページ**（GPU 代ゼロ、根拠付き 5 項目は下の節）。FAQ は AI 向け SEO の FAQ と兼用。
...
7. **SDXL の RTX PRO 6000 品質確認**（ホストが次の実案件で。戻すのは 1 行）。
8. 掃除: Cinematic / Wan Animate 廃止分のデッドコード、巨大 worker ファイルの分割（トークン節約）。
9. ローンチ判断まで保留: AI 向け SEO の残り（llms.txt・機能スペック）、料金ページ、8-② 転送パネル。
