...
- **Multi-Angle の並列実行 追加料金は「混雑料金」として高めにする**（2026-09-23 ホスト方針）。B200 の実コールド原価
  ...
  原価ベースで下げる提案はしない。**形は「通常料金 × 率 + 固定」に変更済み（2026-09-23 夜、ホスト決定「実際の
  クレジットの 2 倍」＝率 1.0）**。knob `angle_priority_parallel_rate`（1.0）を追加、固定分 `angle_priority_parallel_surcharge`
  の既定値は 0 にしたが **DB の既存行は 340 のまま → admin で 0 に下げる（ホスト作業）**。モーダル表記は
  「通常料金に +○C、合計 △C」。マイグレーション `supabase/migrations/20260882000000_angle_parallel_rate_knob.sql`（未適用でも
  既定値 1.0 で動く。admin から率を編集するには適用が必要）。
