...
# ホスト報告への対応。
#
# 【採用目的の明記（2026-09-13、経営判断として記録）】
# 目的は違法コンテンツの生成を可能にすることではなく、プロクリエイターの
# 正当な作画（水着・戦闘シーン・成人向け創作物の非露骨表現等）が、AI側の
# 過剰な安全フィルター（False Positive）によって理不尽に生成拒否/破綻
# させられるのを防ぐことにある。実際の悪用防止は、この text_encoder 差し替え
# ではなく、API 入口側の src/lib/contentPolicy.ts（レッドライン・コンテンツ
# フィルター）が担う — 両者は役割分担であり、後者を弱めてはならない。
#
