...
- **GPU選択制（tier選択UX）構想の実測データ第一号（2026-09-16）**: 「客が時間とコストを天秤にかけて選べるように、機能ごとに使うGPU世代を動的に選択したい」というホスト構想の検証として、超解像動画（`modal_seedvr2_worker.py`）の実際のVRAM使用量を本番相当のCLI（`video_main`）で実測した。
  ...
  - **🚀 明日すぐ動くためのメモ（2026-09-17時点、優先順）**:
    1. **まず`modal_seedvr2_worker.py`に`torch.compile`を適用し、`DEFAULT_BATCH_SIZE`をGPU tier（特にB300）に応じて動的に上げる**（実機測定より先に無料でできる修正——上記「B300が期待したほど速くない理由」参照）。これをやらないと、以降の比較実測が全部不公平なまま。
    2. **修正後、HD(L40S)/2K(H200)/4K(B300)を362frame相当の同条件で再測定**し、今日の数字（梅139C/15.7分・松224C/7分等）が変わるか確認する。
    3. 数字が固まったら、**「速さを金で買う」優先実行オプション**（HD/2Kにも§6の並列課金パターンを追加）をUI・バックエンド実装する。
    4. 余力があれば **H100を実測**（未検証・B300の約56%の単価、H200と同等ならさらにコスパ良い本命候補）。
    5. 2K/4Kの長尺実測（今は72frame単発データのみ、HDで判明した「フレーム数依存はGPUごとに違う」問題が2K/4Kにも当てはまるか要確認）。
