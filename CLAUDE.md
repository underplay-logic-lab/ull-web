...
7. **実行中でも次のジョブを出せるようにする** — `scaledown_window=30` 系は「順番待ち（無料・既定・複数件）／並列実行（追加料金）」の選択制。LoRA も並列は**同じ式の追加料金**（2026-09-26 ホスト判断: GPU 枠の占有を防ぐ混雑料金。順番待ちは無く、終わるのを待てば通常料金）。**並列の追加料金は全タブ共通で「通常料金 × 率（1.0）＋ 固定 50C」**（`src/lib/pricing/parallelSurcharge.ts`。混雑料金なので原価で下げない。新規タブも同じ式・同じ knob 構成）。モーダルは「通常料金に +○C、合計 △C」表記。
