...
- **Read は範囲指定を優先する。** 実測では Read の結果がセッション全体の34%を占め、単発で60万文字（約20万トークン）読んだ例もあった。まず Grep で当たりをつけ、`offset`/`limit` で必要な範囲だけ読む。巨大ファイル（`LoraStudioTab.tsx` 5,500行、`modal_seedvr2_worker.py` 3,600行など）の全文読み込みは原則しない。`modal_lora_worker.py` は 2026-09-24 に `lora_worker_{core,models,train,endpoints}.py` へ分割済み（新モジュールは `_LORA_WORKER_MODULES` に足す）。
...
- **このファイルを肥大化させない。** CLAUDE.md は毎リクエスト送信される固定費。**上限は 35KB**（2026-09-21 にホスト判断で 30KB から引き上げ。現状 約30KB。超えたら必ず `docs/` へ退避する）。実測値・ベンチ結果・事故の経緯など「記録」は `docs/` に置き、ここからは1行で参照する。**ルール（次に同じ場面で守るべきこと）だけをここに書く。**
  - **余白はルールのために使う。** 記録は上限に余裕があっても `docs/` へ置く。逆に、枠を作るために既存ルールの文言を削って精度を落とすのは本末転倒（2026-09-21 に実際にやってしまった）。足りなくなったら削るのは記録であって、ルールの精度ではない。
...
2. **「ジョブが見つからない」は専用エラー** — 14日パージによる不在（`PGRST116`）と通信エラーを区別し、前者は即座に諦めて「見つかりませんでした。新しく生成してください」と案内する（**保持期間は通常 UI に書かない**、2026-09-23 ホスト方針）。
...
7. **実行中でも次のジョブを出せるようにする** — `scaledown_window=30` 系は「順番待ち（無料・既定・複数件）／並列実行（追加料金）」の選択制。`scaledown_window=2` 系（LoRA等）は**追加料金なしで並列を提供**する（どのみち毎回コールドスタートなのでコスト差が無い＝待たせる理由が無い）。**並列の追加料金は全タブ共通で「通常料金 × 率（1.0）＋ 固定 50C」**（`src/lib/pricing/parallelSurcharge.ts`。混雑料金なので原価で下げない。新規タブも同じ式・同じ knob 構成）。モーダルは「通常料金に +○C、合計 △C」表記。
...
11. **結果の URL は使い回さない** — 完了直後の URL は Volume を指し、R2 への移動で 404 になる（R2 の署名も 15 分で切れる）。保存・他タブへの受け渡し・ZIP は押した時点で取り直し、`<img>`/`<video>` は `onError` で取り直す（2 回まで）。URL のキャッシュは署名より短い TTL、保存の失敗は画面に出す（`.catch(() => {})` 禁止）。

これら11点はセットで「Studio タブとして最低限あるべき挙動」なので、新規タブのレビュー・実装時のチェックリストとして扱うこと。
