...
- **量子化禁止**: 推論・学習とも **BF16 フル精度を既定**とし、量子化（fp8 / int8 / int4 / NF4 / GGUF 等）およびモデルオフロード（CPU offload / sequential offload）は原則使用しない。使う場合は実機計測で品質・速度の劣化がないことを示したうえでホスト承認を得ること。
...
5. **生成物は同期/非同期を問わず必ず永続化** — 結果を保存し、ジョブ行を1件残す。新設バケットは `modal_retention_purge.py` の `DEFAULT_BUCKETS` と `src/lib/generatedStorage.ts` の `GENERATED_BUCKETS` の**両方**に追加する（忘れると溜まり続けるか admin から見えない）。
