...
- **Wan2.2-S2V-14B（音声同期動画）実機検証の結論（2026-09-16、`modal_s2v_probe.py`）**: ライセンスは Apache-2.0・地域制限なし・商用利用可（確認済み）で、Director の基盤 MiniMax H3 が除外する米国/EU/英国/韓国（[[minimax-h3-license-risk-accepted]]）でも使える点は価値がある。ただし**単一GPU（B300）では実用速度に届かない**: 5秒クリップで実測 ~9分（約$1.07）・40ステップ・CFGありのeager実行、15秒クリップ（3チャンク自動分割）では ~41分（約$4.90）。コミュニティ報告のH100単体（5秒クリップ4〜17分）とほぼ同水準で、**B300でもH100と大差ない**——この結果は「B300の優位性はVRAM容量であり、compile/FSDPなしのeager単発推論そのものの速度ではない」という Qwen-Image-Edit A100 退行バグ（本セクション上部）の教訓と符合する2件目の実例。複数GPU化（`--ulysses_size`/`--dit_fsdp`）で理論上は短縮できるが、通信オーバーヘッドで収穫逓減する上（4基で~70%効率、8基で~50〜55%効率と仮定すると5秒クリップでも9〜14分止まり）、GPU複数台ぶんのコストが単純に乗る。**現時点の結論**: 本番の汎用動画生成としては不採用（Director/MiniMax H3の体験に届かない）。地域制限回避専用の代替エンジン、または複数GPU前提の高額「時短オプション」としてなら将来検討の余地はあるが、着手する優先度は低い。
  - **教訓: 実機テスト前にコミュニティのベンチマーク報告を先に調べるべきだった**。速度が実用に耐えるかは「実測より先に読める情報がないか確認する」（CLAUDE.md §0）の対象になり得る——今回は事前調査を怠り、依存関係のデバッグ（flash_attn等、CPU課金だけで約$3.72）と2回の的外れなB300実行（`--offload_model` 未指定・プロンプトに "static shot" と書いてしまう等の設定ミスを含む）を経てから気づき、本検証だけで**合計 約$12.36** かかった。
