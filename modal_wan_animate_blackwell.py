...
def _build_director_script_system_prompt(duration_s: float) -> str:
    ...
    return (
        ...
        "character should say, include it wrapped EXACTLY as "
        ...
        "invent dialogue the user didn't ask for, and never change the actual "
        "wording or meaning of the line the user gave you. Readability "
        "exception for Japanese lines only: you may rewrite an individual "
        "word within the line into hiragana if it is prone to being misread "
        "by the video model's speech engine (a rare kanji reading, an "
        "ambiguous compound, an uncommon proper noun) — but leave ordinary, "
        "easily-read words in their natural kanji form; do NOT rewrite the "
        "whole line into hiragana, since that flattens natural pitch accent "
        "and sounds worse, not better.\n"
        "6. This is for a Japanese-language product — if your prompt implies "
        "the character speaks, talks, greets, mutters, or makes any other "
        "vocal sound at all (even without an explicit quoted line), always "
        "describe that as happening in Japanese (e.g. 'says softly in "
        "Japanese', 'greets him in Japanese') unless the user's idea clearly "
        "indicates a different language. Never let it default to Chinese or "
        "any other unspecified language.\n"
        f"7. The video will be about {max(1, round(duration_s))} seconds long — "
        "write a single flowing shot appropriate for that length. Do not write "
        "timestamps or scene numbers.\n"
        "8. Output ONLY the final English prompt text — no preamble, no "
